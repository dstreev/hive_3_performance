...
CREATE EXTERNAL TABLE merge_files_source_2
(
    region      STRING,
    country     STRING,
    report_date TIMESTAMP,
...
ANALYZE TABLE merge_files_source COMPUTE STATISTICS FOR COLUMNS;
...
TRUNCATE TABLE merge_files_part;
...
DISTRIBUTE BY
    ...
    report_date
    , MOD(hash(field8),10)
--     , concat(region, country, report_date)
INSERT OVERWRITE TABLE
    merge_files_part PARTITION (region, country, report_date)
SELECT
    report_date,
...
select count(1) from merge_files_part;
